...
def _get_mlflow_cert_path() -> Optional[str]:
    """Return the TLS CA cert path for MLflow health checks.

    Checks MLFLOW_TRACKING_SERVER_CERT_PATH first (set by the kagenti operator),
    then falls back to REQUESTS_CA_BUNDLE / CURL_CA_BUNDLE.
    Returns None if no custom CA is configured.
    """
    import os

    for env_var in ("MLFLOW_TRACKING_SERVER_CERT_PATH", "REQUESTS_CA_BUNDLE"):
